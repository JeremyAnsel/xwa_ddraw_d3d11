...
PixelShaderInput main(VertexShaderInput input)
{
	...
	output.pos = mul(transformView, output.pos);
	...
	{
		if (output.pos.x < s_V0x0662814)
		{
			if (output.pos.y > s_V0x06626F4)
			{
				output.pos.y = s_V0x06626F4;
			}
		}
		else if (output.pos.x > s_V0x064D1A4)
		{
			if (output.pos.y > s_V0x063D194)
			{
				output.pos.y = s_V0x063D194;
			}
		}
		else
		{
			float esp5C = (output.pos.x - s_V0x0662814) / (s_V0x064D1A4 - s_V0x0662814) * (s_V0x063D194 - s_V0x06626F4) + s_V0x06626F4;

			if (output.pos.y > esp5C)
			{
				output.pos.y = esp5C;
			}
		}
